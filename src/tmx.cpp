...
void TMX::parseOne_task(std::vector<uint8_t> &message)
{
    ...
    // Makes it possible to have longer running callbacks without interfering with other callbacks and reading in data.
    ...
    {
        std::ranges::for_each(this->ping_callbacks, [message](auto callback)
                              { callback(message); });
    }
    break;
    ...
    case TMX::MESSAGE_IN_TYPE::DIGITAL_REPORT:
    {
        ...
        // call the callbacks with the normal message. Should use the other one for normal use.
        std::ranges::for_each(this->digital_callbacks, [message](auto callback)
                              { callback(message); });
    }
    case TMX::MESSAGE_IN_TYPE::ANALOG_REPORT:
    {
        ...
        // call the callbacks with the normal message. Should use the other one for normal use.
        std::ranges::for_each(this->analog_callbacks, [message](auto callback)
                              { callback(message); });
    }
    break;
    case TMX::MESSAGE_IN_TYPE::SERVO_UNAVAILABLE:
    {
        std::ranges::for_each(this->servo_unavailable_callbacks, [&message](auto callback)
                              { callback(message); });
    }
    break;
    case TMX::MESSAGE_IN_TYPE::I2C_WRITE_REPORT:
    {
        std::ranges::for_each(this->i2c_write_callbacks, [&message](auto callback)
                              { callback(message); });
    }
    break;
    case TMX::MESSAGE_IN_TYPE::I2C_READ_FAILED:
    {
        std::ranges::for_each(this->i2c_read_failed_callbacks, [message](auto callback)
                              { callback(message); });
    }
    break;
    case TMX::MESSAGE_IN_TYPE::I2C_READ_REPORT:
    {
        std::ranges::for_each(this->i2c_read_callbacks, [message](auto callback)
                              { callback(message); });
    }
    break;
    case TMX::MESSAGE_IN_TYPE::SONAR_DISTANCE:
    {
        ...
        std::ranges::for_each( // filter, get callback, call callback
            ...
            [pin, value](auto callback)
            { callback(pin, value); });
        std::ranges::for_each(this->sonar_distance_callbacks, [message](auto callback)
                              { callback(message); });
    }
    break;
    case TMX::MESSAGE_IN_TYPE::ENCODER_REPORT:
    {
        ...
        std::ranges::for_each( // filter, get callback, call callback
            ...
            [pin, value](auto callback)
            { callback(pin, value); });
        std::ranges::for_each(this->encoder_callbacks, [message](auto callback)
                              { callback(message); });
    }
    case TMX::MESSAGE_IN_TYPE::DEBUG_PRINT:
    {
        std::ranges::for_each(this->debug_print_callbacks, [message](auto callback)
                              { callback(message); });
    }
    default:
        break;
...
void TMX::setPinMode(uint8_t pin, TMX::PIN_MODES mode, bool reporting, uint16_t analog_differential)
{
    ...
    case TMX::PIN_MODES::ANALOG_INPUT:
        message = std::vector<uint8_t>{pin, mode, (uint8_t)((analog_differential >> 8) & 0xff), (uint8_t)(analog_differential & 0xff), reporting};
        break;
    default:
        break;
    }
...
void TMX::add_callback(MESSAGE_IN_TYPE type, std::function<void(std::vector<uint8_t>)> callback)
{
    switch (type)
    {
    case TMX::MESSAGE_IN_TYPE::PONG_REPORT:
...
void TMX::attach_encoder(uint8_t pin_A, uint8_t pin_B, std::function<void(uint8_t, int8_t)> callback)
{
    this->encoder_callbacks_pin.push_back({pin_A, callback});
    this->sendMessage(TMX::MESSAGE_TYPE::ENCODER_NEW, {pin_A, pin_B});
}

void TMX::attach_sonar(uint8_t trigger, uint8_t echo, std::function<void(uint8_t, uint16_t)> callback)
{
